  ...
  require 'net/http'
    require 'net/https'
  class Alarm < ExternalUser#ApplicationRecord
    acts_as_paranoid
    belongs_to :machine
  ...
  #puts "this tenant was no Setting"
   ...
   def self.notification_test(machine_log)

        #machine_log = MachineLog.find(18249414)
        machine = machine_log.machine.id
        machine_name = machine_log.machine.machine_name
        # law of demeter in bestpractices
       ...
       if machine_log.machine.set_alarm_settings.present?
         ...
         player_id = OneSignal.where(user_id:machine_log.machine.tenant.users).pluck(:player_id).uniq
         if player_id.present?
          
          if machine_log.machine_status == 1 || machine_log.machine_status == 100 || machine_log.machine_status == 0
             if machine_log.machine_status == 100
             status = "stop"
             end
            if machine_log.machine_status == 1 || machine_log.machine_status == 0
               status = "idle"
            #   status = "Running"
            end
             
            alarm_time = machine_log.machine.set_alarm_settings.where(active: true)
            #byebug
            alarm_time.each do |alarm|
             
              if alarm.time_interval != nil && alarm.active == true && alarm.alarm_for == status

                message = machine_name + ":" + status
                if Notification.where(message: message, machine_id:machine).count == 0
                 if machine_log.machine.machine_logs.where(machine_status: '3').last.created_at + alarm.time_interval.to_i.minutes < Time.now                
                  Notification.create(machine_log_id: machine_log.id,message: message,machine_id:machine)
                 # params = {"app_id" => "61c6c36b-9640-4d15-8b00-2a9a4559b626",
                ...
                elsif Notification.where(message: message, machine_id: machine).order(:id).last.created_at + alarm.time_interval.to_i.minutes < Time.now
     ...
     puts "No notification avaliable"
  end
end
