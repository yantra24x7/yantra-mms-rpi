...
def alarm_last_history # Last 50
  mac = Machine.find_by_machine_ip(params[:machine_id])
  if mac.alarm_tests.where(alarm_type: params[:alarm_type], alarm_no: params[:alarm_no], axis_no: params[:axis_no], time: params[:time], message:params[:message], category: params[:alarm_category]).last.present?
    puts "Old Alarms"
  else
    AlarmTest.create(alarm_type: params[:alarm_type], alarm_no: params[:alarm_no], axis_no: params[:axis_no], time: params[:time], message:params[:message], alarm_status:params[:alarm_status], machine_id:mac.id, category: params[:alarm_category])
  end  
end
